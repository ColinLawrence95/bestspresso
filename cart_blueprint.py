...
def update_cart_item(id):
    ...
    try:
        ...
        cursor.execute(""" exists and belongs to the user's ca
        ...
        cursor.execute("UPDATE cart_items SET quantity = %s WHERE id = %s", (quantity, id))
        connection.commit()
        cursor.close()
        connection.close()
...
def purchase_cart():
    ...
    try:
        ...
        cur.execute("""
            SELECT ci.id, ci.product_id, p.price, ci.quantity, p.stock
            FROM carts c
            JOIN cart_items ci ON c.id = ci.cart_id
            JOIN products p ON ci.product_id = p.id
            WHERE c.user_id = %s
        ...
        total = sum(float(row[2]) * row[3] for row in items)

        cur.execute("SELECT balance FROM users WHERE id = %s", (user_id,))
        balance = cur.fetchone()
        if not balance or float(balance[0]) < total:
            cur.close()
            conn.close()
            return jsonify({'error': 'Insufficient balance'}), 400

        new_balance = float(balance[0]) - total
        cur.execute("UPDATE users SET balance = %s WHERE id = %s", (new_balance, user_id))

        for item in items:
            cart_item_id, product_id, price, quantity, stock = item
